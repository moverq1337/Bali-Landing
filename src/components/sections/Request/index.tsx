...
// Тип для кастомного Button (если он у тебя свой)
...
const Request: React.FC<RequestProps> = ({ className }) => {
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: name === 'adults' || name === 'children' ? Number(value) || 0 : value,
    }));
  };
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('http://91.108.245.202:7777/submit-request', {
        method: 'POST',
        headers: {
    ...
    <section className={`px-4 md:px-8 py-8 md:py-16 ${className}`}>
      <div className="max-w-2xl mx-auto">
        ...
        <p className="text-center text-gray-600 mb-8 font-['Montserrat']">
          Заявка автоматически попадет к менеджеру турагентства и с Вами свяжутся в ближайшее время
        </p>
...
<form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500">
            Нажимая на кнопку «Отправить заявку», я соглашаюсь с политикой обработки персональных данных
          </div>
          <Button
            type="submit"
...
// Мокаем компонент Button, если он не импортируется (замени на свой импорт)
const Button: React.FC<ButtonProps> = ({ type, className, variant, size, children }) => (
  <button
    type={type}
